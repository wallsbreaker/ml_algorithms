...
class RegressionTree(object):
    def __init__(self, sample_num_sentry):
    ...
    def construct_node(self, train_data_x, train_data_y):
        ...
        if len(set(train_data_y)) == 1 or len(train_data_x) < self.sample_num_sentry:
            ...
            #该节点选择的特征
            best_feature_ix, best_feature_value = self.select_feature(train_data_x, train_data_y)
    ...
    def select_feature(self, train_data_x, train_data_y):
        ...
        for ix in xrange(len(train_data_x[0])):
            ...
            for feature_value in feature_candidates[:-1]:
                tmp_loss = self.loss_by_split(train_data_x, train_data_y, ix, feature_value)
                if tmp_loss < min_loss:
                    min_feature = ix
                    min_feature_value = feature_value

        return min_feature, min_feature_value
